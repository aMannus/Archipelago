...
def set_region_rules(world: "SohWorld") -> None:
    ...
    add_locations(Regions.ZORAS_FOUNTAIN, world, [
        ...
        (Locations.ZF_GS_ABOVE_THE_LOG,
         lambda bundle: is_child(bundle) & hookshot_or_boomerang(bundle) & can_get_nighttime_gs(bundle)),
    ...
    connect_regions(Regions.ZORAS_FOUNTAIN, world, [
        (Regions.ZD_BEHIND_KING_ZORA, lambda bundle: True_()),
        ...
        (Regions.ZF_HIDDEN_CAVE, lambda bundle: can_use(
            Items.SILVER_GAUNTLETS, bundle) & blast_or_smash(bundle)),
    ...
    add_locations(Regions.ZF_LAKEBED, world, [
        (Locations.ZF_BOTTOM_FREESTANDING_POH,
         lambda bundle: is_adult(bundle) & can_use(Items.IRON_BOOTS, bundle) & water_timer_at_least(bundle, 16)),
    ...
    connect_regions(Regions.ZF_HIDDEN_CAVE, world, [
        (Regions.ZF_HIDDEN_LEDGE, lambda bundle: True_())
    ])

    # Zora's Fountain Hidden Ledge
    # Locations
    add_locations(Regions.ZF_HIDDEN_LEDGE, world, [
        (Locations.ZF_GS_HIDDEN_CAVE, lambda bundle: is_adult(bundle) & can_get_enemy_drop(
            bundle, Enemies.GOLD_SKULLTULA, EnemyDistance.BOMB_THROW) & can_get_nighttime_gs(bundle))
    ])
    # Connections
    connect_regions(Regions.ZF_HIDDEN_LEDGE, world, [
        (Regions.ZORAS_FOUNTAIN, lambda bundle: has_item(
            Items.BRONZE_SCALE, bundle) | take_damage(bundle)),
        (Regions.ZF_HIDDEN_CAVE, lambda bundle: True_())
    ])
