...
def set_region_rules(world: "SohWorld") -> None:
    ...
    connect_regions(Regions.DMC_UPPER_NEARBY, world, [
        ...
        (Regions.DMC_UPPER_GROTTO,
         lambda bundle: blast_or_smash(bundle) and (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)),
    ])

    # Death Mountain Crater Upper Local
    # Events
    add_events(Regions.DMC_UPPER_LOCAL, world, [
        (EventLocations.DMC_GOSSIP_STONE_FAIRY, Events.CAN_ACCESS_FAIRIES,
         lambda bundle: has_explosives(bundle)
         and call_gossip_fairy_except_suns(bundle)
         and (fire_timer(bundle) >= 16 or hearts(bundle) >= 3))
    ])
    # Locations
    add_locations(Regions.DMC_UPPER_LOCAL, world, [
        ...
        (Locations.DMC_GS_CRATE,
         lambda bundle: (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)
         and is_child(bundle) and can_attack(bundle) and can_break_crates(bundle)),
        (Locations.DMC_GOSSIP_STONE_FAIRY,
         lambda bundle: call_gossip_fairy_except_suns(bundle)
         and has_explosives(bundle) and (fire_timer(bundle) >= 16 or hearts(bundle) >= 3)),
        (Locations.DMC_GOSSIP_STONE_BIG_FAIRY,
         lambda bundle: can_use(Items.SONG_OF_STORMS, bundle) and (fire_timer(bundle) >= 16 or hearts(bundle) >= 3)),
        (Locations.DMC_CRATE,
         lambda bundle: (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)
         and is_child(bundle) and can_break_crates(bundle))
    ])
    # Connections
    connect_regions(Regions.DMC_UPPER_LOCAL, world, [
        ...
        (Regions.DMC_CENTRAL_NEARBY,
         lambda bundle: is_adult(bundle) and can_use(Items.GORON_TUNIC, bundle)
         and can_use(Items.DISTANT_SCARECROW, bundle)
         and (effective_health(bundle) > 2 or (can_use(Items.BOTTLE_WITH_FAIRY, bundle)
                                               and False or can_use(Items.NAYRUS_LOVE, bundle)))),  # TODO Implement Dungeon Shuffle Option to replace False
        (Regions.DMC_LOWER_NEARBY, lambda bundle: False),
        (Regions.DMC_DISTANT_PLATFORM,
         lambda bundle: (fire_timer(bundle) >= 48 or hearts(bundle) >= 2) or hearts(bundle) >= 3),
    ])
    ...
    connect_regions(Regions.DMC_LADDER_REGION_NEARBY, world, [
        ...
        (Regions.DMC_LOWER_NEARBY,
         lambda bundle: hearts(bundle) >= 3
         and (can_use(Items.HOVER_BOOTS, bundle) or (can_do_trick("DMC Boulder JS", bundle)
                                                     and is_adult(bundle) and can_use(Items.MEGATON_HAMMER, bundle))
              or (can_do_trick("DMC Boulder Skip", bundle) and is_adult(bundle))))
    ])
    ...
    connect_regions(Regions.DMC_LOWER_LOCAL, world, [
        ...
        (Regions.DMC_CENTRAL_LOCAL,
         lambda bundle: (can_use(Items.HOVER_BOOTS, bundle) or can_use(Items.HOOKSHOT, bundle)
                         or (is_adult(bundle) and can_shield(bundle) and can_do_trick("DMC Bolero Jump", bundle)))
         and fire_timer(bundle) >= 24)
    ])
    ...
    add_locations(Regions.DMC_CENTRAL_NEARBY, world, [
        (Locations.SHEIK_IN_CRATER,
         lambda bundle: is_adult(bundle) and (fire_timer(bundle) >= 8 or hearts(bundle) >= 3))
    ])
    # Connections
    connect_regions(Regions.DMC_CENTRAL_NEARBY, world, [
        (Regions.DMC_CENTRAL_LOCAL, lambda bundle: fire_timer(bundle) >= 48),
        (Regions.DMC_VOLCANO_VENT,
         lambda bundle: is_adult(bundle) and hearts(bundle) >= 3 and can_do_trick("DMC Hover Bean POH", bundle)
         and can_use(Items.HOVER_BOOTS, bundle)),
    ...
    add_events(Regions.DMC_CENTRAL_LOCAL, world, [
        (EventLocations.DMC_BEAN_PLANT_FAIRY, Events.CAN_ACCESS_FAIRIES,
         lambda bundle: can_use(Items.MAGIC_BEAN, bundle) and can_use(Items.SONG_OF_STORMS, bundle)
         and (fire_timer(bundle) >= 8 or hearts(bundle) >= 3))
    ])
    # Locations
    add_locations(Regions.DMC_CENTRAL_LOCAL, world, [
        (Locations.DMC_GS_BEAN_PATCH,
         lambda bundle: (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)
         and can_spawn_soil_skull(bundle) and can_attack(bundle)),
        (Locations.DMC_NEAR_WARP_PLATFORM_RED_RUPEE, lambda bundle: is_child(bundle)),
        (Locations.DMC_MIDDLE_PLATFORM_RED_RUPEE,
         lambda bundle: is_child(bundle) and (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)),
        ...
        (Locations.DMC_BEAN_SPROUT_FAIRY1,
         lambda bundle: is_child(bundle) and can_use(Items.MAGIC_BEAN, bundle) and can_use(Items.SONG_OF_STORMS, bundle)
         and (fire_timer(bundle) >= 8 or hearts(bundle) >= 3)),
    ...
    connect_regions(Regions.DMC_CENTRAL_LOCAL, world, [
        (Regions.DMC_CENTRAL_NEARBY, lambda bundle: True),
        (Regions.DMC_LOWER_NEARBY,
         lambda bundle: (is_adult(bundle) and can_plant_bean(bundle))
         or can_use(Items.HOVER_BOOTS, bundle) or can_use(Items.HOOKSHOT, bundle)),
        (Regions.DMC_UPPER_NEARBY, lambda bundle: is_adult(bundle) and can_plant_bean(bundle)),
        (Regions.FIRE_TEMPLE_ENTRYWAY,
         lambda bundle: (is_child(bundle) and hearts(bundle) >= 3 and False)
         or (is_adult(bundle) and fire_timer(bundle) >= 24)),  # TODO Implement Dungeon Shuffle Option to replace False
        (Regions.DMC_DISTANT_PLATFORM,
         lambda bundle: (fire_timer(bundle) >= 48 or hearts(bundle) >= 2) and can_use(Items.DISTANT_SCARECROW, bundle)),
    ...
    connect_regions(Regions.DMC_DISTANT_PLATFORM, world, [
        (Regions.DMC_CENTRAL_LOCAL,
         lambda bundle: fire_timer(bundle) >= 48 and can_use(Items.DISTANT_SCARECROW, bundle))
    ])
