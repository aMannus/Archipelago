...
def set_region_rules(world: "SohWorld") -> None:
    ...
    add_events(Regions.SHADOW_TEMPLE_BEGINNING, world, [
        (EventLocations.SHADOW_TEMPLE_BEGINNING_NUT_POT, Events.CAN_FARM_NUTS, lambda bundle: True)
    ])
    # Locations
    add_locations(Regions.SHADOW_TEMPLE_BEGINNING, world, [
        ...
        (Locations.SHADOW_TEMPLE_WHISPERING_WALLS_POT3, lambda bundle: can_break_pots(bundle)),
        (Locations.SHADOW_TEMPLE_WHISPERING_WALLS_POT4, lambda bundle: can_break_pots(bundle)),
        (Locations.SHADOW_TEMPLE_WHISPERING_WALLS_POT5, lambda bundle: can_break_pots(bundle)),
        (Locations.SHADOW_TEMPLE_MAP_CHEST_POT1, lambda bundle: can_break_pots(bundle)),
        (Locations.SHADOW_TEMPLE_MAP_CHEST_POT2, lambda bundle: can_break_pots(bundle))
    ])
    # Connections
    connect_regions(Regions.SHADOW_TEMPLE_BEGINNING, world, [
        (Regions.SHADOW_TEMPLE_ENTRYWAY, lambda bundle: True),
        (Regions.SHADOW_TEMPLE_FIRST_BEAMOS, lambda bundle: can_use(Items.HOVER_BOOTS, bundle)),
    ])
    ...
    add_locations(Regions.SHADOW_TEMPLE_FIRST_BEAMOS, world, [
        (Locations.SHADOW_TEMPLE_COMPASS_CHEST, lambda bundle: can_jump_slash_except_hammer(bundle)),
        (Locations.SHADOW_TEMPLE_EARLY_SILVER_RUPEE_CHEST, lambda bundle: can_use(Items.HOVER_BOOTS, bundle) or can_use(Items.HOOKSHOT, bundle)),
        (Locations.SHADOW_TEMPLE_BEAMOS_SONG_OF_STORMS_FAIRY, lambda bundle: can_use(Items.SONG_OF_STORMS, bundle))
    ])
    # Connections
    connect_regions(Regions.SHADOW_TEMPLE_FIRST_BEAMOS, world, [
        (Regions.SHADOW_TEMPLE_HUGE_PIT, lambda bundle: has_explosives(bundle) and is_adult(bundle) and small_keys(Items.SHADOW_TEMPLE_SMALL_KEY, 1, bundle)),
    ...
    add_locations(Regions.SHADOW_TEMPLE_HUGE_PIT, world, [
        ...
        (Locations.SHADOW_TEMPLE_GS_FALLING_SPIKES_ROOM, lambda bundle: can_use(Items.HOOKSHOT, bundle) or (can_do_trick(Tricks.SHADOW_UMBRELLA_GS, bundle) and can_use(Items.HOVER_BOOTS, bundle) and can_standing_shield(bundle) and can_use(Items.MASTER_SWORD, bundle)) or (is_adult(bundle) and can_ground_jump(bundle))),
    ...
    add_locations(Regions.SHADOW_TEMPLE_WIND_TUNNEL, world, [
        ...
        (Locations.SHADOW_TEMPLE_AFTER_WIND_ENEMY_CHEST, lambda bundle: can_kill_enemy(bundle, Enemies.GIBDO, EnemyDistance.CLOSE, True, 2)),
        (Locations.SHADOW_TEMPLE_AFTER_WIND_HIDDEN_CHEST, lambda bundle: has_explosives(bundle)),
        (Locations.SHADOW_TEMPLE_GS_NEAR_SHIP, lambda bundle: can_use(Items.LONGSHOT, bundle) and small_keys(Items.SHADOW_TEMPLE_SMALL_KEY, 4, bundle)),
    ...
    connect_regions(Regions.SHADOW_TEMPLE_WIND_TUNNEL, world, [
        (Regions.SHADOW_TEMPLE_BEYOND_BOAT, lambda bundle: can_jump_slash_except_hammer(bundle) and can_use(Items.ZELDAS_LULLABY, bundle) and small_keys(Items.SHADOW_TEMPLE_SMALL_KEY, 4, bundle)),
    ])

    ## Shadow Temple Beyond Boat
    # Locations
    add_locations(Regions.SHADOW_TEMPLE_BEYOND_BOAT, world, [
        (Locations.SHADOW_TEMPLE_SPIKE_WALLS_LEFT_CHEST, lambda bundle: can_use(Items.DINS_FIRE, bundle)),
        (Locations.SHADOW_TEMPLE_BOSS_KEY_CHEST, lambda bundle: can_use(Items.DINS_FIRE, bundle)),
        (Locations.SHADOW_TEMPLE_INVISIBLE_FLOORMASTER_CHEST, lambda bundle: can_kill_enemy(bundle, Enemies.FLOORMASTER)),
    ...
    connect_regions(Regions.SHADOW_TEMPLE_BOSS_ENTRYWAY, world, [
        (Regions.SHADOW_TEMPLE_BEYOND_BOAT, lambda bundle: False),
        (Regions.SHADOW_TEMPLE_BOSS_ROOM, lambda bundle: has_item(Items.SHADOW_TEMPLE_BOSS_KEY, bundle))
    ])
